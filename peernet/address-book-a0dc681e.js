      ...
      for (const transport of transports) {
        for (var protocol of protocols) {
          if (transport.name && !globalThis.navigator) {
            ...
            this.addresses[transport.name] = `/${transport.name}/${transport.port}/${this.ip}/${protocol}/${peerId}`;
          } else {
            this.addresses[transport] = `/${transport}/${this.ip}/${protocol}/${peerId}`;
          }
        }  
      }    
    } else {
      this.addresses['ws'] = `/peernet/${peerId}/disco-swarm`;
    }
    return this
  }
