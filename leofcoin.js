...
var HDNode = _interopDefault(require('bitcoinjs-lib/src/hdnode'));
var MultiSignature = _interopDefault(require('multi-signature'));
...
const signalServers = (() => {
  if (network === 'olivia') return [
    '/ip4/162.208.10.171/tcp/4002/ipfs/QmYSZtP9nSRmrzgmR7nQCStpcRmjE5jPQPUz6s1gySrE2q'
  ]
  else return [
...
const seed = 'https://septimal-balinese-2547.dataplicity.io';
const configPath = path.join(AppData, 'core.config');
const reward = 150;
const consensusSubsidyInterval = 52500;
const consensusSubsidyPercentage = 12.5;
...
const connect = (peers = []) => new Promise(async (resolve, reject) => {
  try {
    __chunk_1.bus.emit('connecting', true);
    debug('connecting peers');
    const { id: id$$1 } = await ipfs.id();
    peers = await getPeerAddresses();
    peers = peers.map(peer => {
      if (!peer.includes(id$$1)) return peer;
    });
    await _connect(peers);
    succes(`connected to ${peers.length} peer(s)`);
    ipfs.pubsub.publish('peer-connected', Buffer.from(id$$1));
    __chunk_1.bus.emit('connecting', false);
    resolve();
  } catch (e) {
    reject(e);
  }
...
class HDWallet {
	constructor(network, hdnode) {
		if (typeof network === 'string') this.network = networks$1[network];
	...
	generate(network) {
		network = networks$1[network] || this.network;
...
class MultiHDWallet extends HDWallet {
	constructor(network, hdnode) {
	...
	encode() {
		...
		const buffer = Buffer.concat([
			...
			Buffer.from(bs58.decode(base58))
		]);
	...
	derive(path$$1) {
		return new MultiHDWallet(this.network, this.hdnode.derivePath(path$$1));
	}
}
